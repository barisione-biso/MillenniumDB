...
# Ignored tests
IGNORED_BAD_QUERIES = ["not_well_designed_1.rq", "not_well_designed_2.rq"]
...
def kill_server(server_process):
    ...
    server_process.kill()
    server_process.wait()
    print_log("KILL_SERVER()", type="end")
...
def query_good(query_file, server_process):
    """Returns True if the query output matches the expected json, False otherwise"""
    if server_process.poll() is not None:
        print_log("Server process has terminated unexpectedly!", type="error")
        sys.exit(1)

    response = query(open(query_file, "r").read())
    response_json = response.convert()
    expected_json_file = query_file.replace(".rq", ".json")
    return response_json == json.load(open(expected_json_file, "r"))
    ...
    for query_category_dir in glob.glob(to_find):
        ...
        for query_file in glob.glob(to_find):
            ...
            if query_name in IGNORED_BAD_QUERIES:
                continue
            error_thrown = query_bad(query_file, server_process)
            # Bad queries should throw an error
            if error_thrown is False:
...
def execute_queries():
    ...
    for test_dir in test_dirs:
        ...
        count_ok_bad, count_error_bad = execute_bad_queries(
            bad_queries_dir, server_process
        )
        total_count_ok += count_ok_bad
        total_count_error += count_error_bad

        good_queries_dir = os.path.join(test_dir, "good_queries")
        count_ok_good, count_error_good = execute_good_queries(
            good_queries_dir, server_process
        )
