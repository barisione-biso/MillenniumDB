...
- AStar Iter: AStar implementation. Uses an Iter, but each node has his itself iter. Has a better perfomance with smaller outputs. An optimization that this algorithm may have, is the steps used to modify a attribute of the priority queue.

- DFS Iter: Implements DFS with iter, similar to AStar. The main problem with this algorithms is that the paths that it founds are not optimal, in some cases longer paths can affect the answer time due to time used to construct it.

Another important thing is that you have to understand that when a state is visited, it will never be visited again. By this reason, in some cases all posible paths will not returned, only the shortest subpath that is solution, if there are not a valid subpath, then the path will be returned. Example:

If the following paths are valid:

(Q0)=[:P0]=>(Q1)=[:P1]=>(Q2)

(Q0)=[:P0]=>(Q1)=[:P1]=>(Q2)=[:P0]=>(Q1)=[:P1]=>(Q2)

The second path will not be returned, because exists a valid subpath, so when Q2 is visited, Q1 will not be added, becasue already has been visited by a connection with the same label.