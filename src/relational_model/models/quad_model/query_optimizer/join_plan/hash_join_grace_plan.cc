#include "hash_join_grace_plan.h"
#include "relational_model/execution/binding_id_iter/hash_join/hash_join_grace.h"

using namespace std;

HashJoinGracePlan::HashJoinGracePlan(unique_ptr<JoinPlan> _lhs, unique_ptr<JoinPlan> _rhs) :
...
HashJoinGracePlan::HashJoinGracePlan(const HashJoinGracePlan& other) :
...
unique_ptr<JoinPlan> HashJoinGracePlan::duplicate() {
    return make_unique<HashJoinGracePlan>(*this);
}


void HashJoinGracePlan::print(int indent, bool estimated_cost, std::vector<std::string>& var_names) {
...
double HashJoinGracePlan::estimate_cost() {
...
double HashJoinGracePlan::estimate_output_size() {
    // return lhs->estimate_output_size() * rhs->estimate_output_size();
    return output_size;
}


uint64_t HashJoinGracePlan::get_vars() {
    return lhs->get_vars() | rhs->get_vars();
}


void HashJoinGracePlan::set_input_vars(uint64_t /*input_var_order*/) {
...
unique_ptr<BindingIdIter> HashJoinGracePlan::get_binding_id_iter(ThreadInfo* thread_info) {
