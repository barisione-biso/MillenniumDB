#include "hash_join_in_memory_plan.h"
#include "relational_model/execution/binding_id_iter/hash_join/hash_join_in_memory.h"

using namespace std;

HashJoinInMemoryPlan::HashJoinInMemoryPlan(unique_ptr<JoinPlan> _lhs, unique_ptr<JoinPlan> _rhs) :
...
HashJoinInMemoryPlan::HashJoinInMemoryPlan(const HashJoinInMemoryPlan& other) :
...
unique_ptr<JoinPlan> HashJoinInMemoryPlan::duplicate() {
    return make_unique<HashJoinInMemoryPlan>(*this);
}


void HashJoinInMemoryPlan::print(int indent, bool estimated_cost, std::vector<std::string>& var_names) {
...
double HashJoinInMemoryPlan::estimate_cost() {
...
double HashJoinInMemoryPlan::estimate_output_size() {
    // return lhs->estimate_output_size() * rhs->estimate_output_size();
    return output_size;
}


uint64_t HashJoinInMemoryPlan::get_vars() {
    return lhs->get_vars() | rhs->get_vars();
}


void HashJoinInMemoryPlan::set_input_vars(uint64_t /*input_var_order*/) {
...
unique_ptr<BindingIdIter> HashJoinInMemoryPlan::get_binding_id_iter(std::size_t binding_size) {
