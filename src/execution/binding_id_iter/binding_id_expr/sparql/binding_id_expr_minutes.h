...
class BindingIdExprMinutes : public BindingIdExpr {
public:
    std::unique_ptr<BindingIdExpr> expr;

    BindingIdExprMinutes(std::unique_ptr<BindingIdExpr> expr) :
        expr (std::move(expr)) { }
    ...
    ObjectId eval(const BindingId& binding_id) const override {
        ...
        if (expr_oid.id & (1ULL << 54)) {
            // Precision bit is set, return default value as minutes is not stored
            return Conversions::pack_int(0);
        } else {
            // Precision bit is not set, minutes is stored normally
            uint64_t minutes = (expr_oid.id & 0x0000'0000'00FC'0000) >> 18;
            return Conversions::pack_int(minutes);
        }
    }
};
