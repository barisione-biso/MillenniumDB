...
class BindingIdExprMonth : public BindingIdExpr {
public:
    std::unique_ptr<BindingIdExpr> expr;

    BindingIdExprMonth(std::unique_ptr<BindingIdExpr> expr) :
        expr (std::move(expr)) { }
    ...
    ObjectId eval(const BindingId& binding_id) const override {
        ...
        if (expr_oid.id & (1ULL << 54)) {
            // Precision bit is set, return default value as month is not stored
            return Conversions::pack_int(1);
        } else {
            // Precision bit is not set, month is stored normally
            uint64_t month = (expr_oid.id & 0x0000'003C'0000'0000) >> 34;
            return Conversions::pack_int(month);
        }
    }
};
